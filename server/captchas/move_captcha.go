...
func (c *captcha) OutImgEncodeString() (error, string, string) {
	...
	baseBufIm := fmt.Sprintf("data:%s;base64,%s", "image/png", base64.StdEncoding.EncodeToString(bufIm))
	baseBufImSlide := fmt.Sprintf("data:%s;base64,%s", "image/png", base64.StdEncoding.EncodeToString(bufImSlide))
	return err, baseBufIm, baseBufImSlide
}
